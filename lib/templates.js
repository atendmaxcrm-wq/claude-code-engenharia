...
function buildManifest(config, packageRoot) {
  ...
  // Rules
  const rules = ['workflow-engineering.md', 'memoria-contextual.md', 'api-backend.md', 'component-architecture.md', 'frontend-react.md', 'services-types.md'];
  ...
  const commands = [
    ...
    'deploy.md', 'agent-teams.md', 'clonar-design.md', 'scraping.md',
  ];
  ...
  if (config.module === 'standard' || config.module === 'full') {
    ...
    const skillCategories = {
      core: [
        ...
        'verification-before-completion', 'performance-audit', 'agent-teams',
      ],
      frontend: [
      ...
      workflow: [
        ...
        'ai-agent-prompt-builder', 'ai-agent-prompt-builder-gemini', 'text-humanizer',
        'atualizar-memoria', 'nova-feature', 'migrar-componente',
        'investigar-bug', 'pesquisar-tech', 'resumo-sessao', 'deploy',
        'entrega-verificada', 'testar-agente',
      ],
    };

    // Top-level skills (no category folder) — each lives directly at templates/skills/<skill>/SKILL.md
    const topLevelSkills = [
      'scraping', 'scraping-insta',
      'clonar-site', 'clonar-design',
      'gsap-animations', 'upgrade-visual', 'criacao-form',
      'site-elite', 'site-elite-cinema', 'replicar-sistema', 'auditar-fidelidade',
      'extrair-design-imagem', 'capturar-logica-saas',
    ];
    ...
    for (const skill of topLevelSkills) {
      const skillSrc = path.join(templatesDir, 'skills', skill);
      if (fs.existsSync(skillSrc)) {
        const skillFiles = walkDir(skillSrc);
        for (const sf of skillFiles) {
          const relInSkill = path.relative(skillSrc, sf);
...
function renderTemplates(config, packageRoot, options = {}) {
  ...
  const vars = {
    ...
    embedding_provider: config.geminiKey ? 'gemini' : (config.openaiKey ? 'openai' : 'gemini'),
