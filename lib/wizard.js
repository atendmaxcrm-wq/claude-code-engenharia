...
async function runWizard(opts) {
  ...
  // Non-interactive mode: --yes (or --quiet + --module) skips the wizard entirely.
  // Uses sensible defaults for every field; DB/OpenAI come from env vars when available.
  if (opts.yes || (opts.quiet && opts.module)) {
    config.module = opts.module || 'full';
    config.stackDescription = process.env.CCE_STACK || '';
    config.healthCheckCommand = process.env.CCE_HEALTH_CHECK || 'curl http://localhost:3000/health';
    config.logCommand = process.env.CCE_LOG_COMMAND || 'pm2 logs --lines 20';
    if (config.module === 'full') {
      config.dbHost = process.env.CCE_DB_HOST || config.dbHost;
      config.dbPort = process.env.CCE_DB_PORT || config.dbPort;
      config.dbUser = process.env.CCE_DB_USER || config.dbUser;
      config.dbPassword = process.env.CCE_DB_PASSWORD || '';
      config.dbName = process.env.CCE_DB_NAME || config.dbName;
      config.geminiKey = process.env.GEMINI_API_KEY || process.env.CCE_GEMINI_KEY || '';
      config.openaiKey = process.env.OPENAI_API_KEY || process.env.CCE_OPENAI_KEY || '';
    }
    return config;
  }
  ...
  try {
    ...
    if (config.module === 'full') {
      ...
      console.log('');
      console.log('  Embeddings: por padrao usamos Gemini (gemini-embedding-001, 1536 dims).');
      config.geminiKey = await ask(rl, 'Gemini API Key (para embeddings)', '');
      config.openaiKey = await ask(rl, 'OpenAI API Key (fallback, opcional)', '');
    }
  } finally {
    rl.close();
  }
