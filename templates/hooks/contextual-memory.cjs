...
const SESSION_CACHE_DIR = '/tmp/aios-memory-sessions';
...
/**
 * Decide se vale a pena buscar memorias para este prompt
 */
function shouldSearch(prompt) {
  ...
  // Slash commands do Claude Code (/help, /clear, /compact, etc.)
  if (/^\/[a-z-]+$/i.test(trimmed)) return false;
  return true;
}

/**
 * Extrai keywords relevantes do prompt (remove stopwords)
 */
...
/**
 * Busca rapida por keywords via ILIKE com scoring
 */
...
/**
 * Busca semantica com timeout (race pattern)
 */
async function semanticSearchWithTimeout(searchSimilar, prompt, timeoutMs) {
  ...
  ]).catch(() => null); // Timeout ou erro: retorna null (graceful)
}

/**
 * Merge resultados keyword + semantico, dedup por ID
 */
function mergeResults(kwResults, semResults, limit) {
  const seen = new Set();
  const merged = [];

  // Prioriza resultados semanticos (melhor relevancia)
  ...
  // Complementa com keyword results
...
/**
 * Cache de sessao — evita reinjetar mesma memoria no mesmo prompt
 */
...
/**
 * Formata output para injecao no contexto do Claude
 */
...
function loadEnvFromMcp() {
  try {
    ...
    const env = mcpConfig.mcpServers['aios-memory'].env;
...
async function main() {
  ...
  // Short-circuit para mensagens triviais
  if (!shouldSearch(prompt)) {
    process.exit(0);
  }

  // Carrega modulos de memoria
  ...
  try {
    ...
    // Executa keyword search (rapido) e semantic search (race) em paralelo
    ...
    // Filtra ja injetados nesta sessao
    ...
    // Log erro mas NUNCA bloqueia Claude Code
